...
void cilkmerge_par(ELM *low1, ELM *high1, ELM *low2, ELM *high2, ELM *lowdest, int *omptr_bb_id_ptr)
{
     OMPTR_NEW_CONTEXT();
     /*
...
{
     OMPTR_TASK_START();
     cilkmerge_par(low1, split1 - 1, low2, split2, lowdest, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
OMPTR_BEFORE_TASK();
...
void cilksort_par(ELM *low, ELM *tmp, long size, int* omptr_bb_id_ptr)
{
     /*
...
{
     OMPTR_TASK_START();
     cilksort_par(A, tmpA, quarter, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
OMPTR_BEFORE_TASK();
...
{
     OMPTR_TASK_START();
     cilksort_par(B, tmpB, quarter, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
OMPTR_BEFORE_TASK();
...
{
     OMPTR_TASK_START();
     cilksort_par(C, tmpC, quarter, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
OMPTR_BEFORE_TASK();
...
{
     OMPTR_TASK_START();
     cilksort_par(D, tmpD, size - 3 * quarter, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
OMPTR_BEFORE_TASKWAIT();
...
{
     OMPTR_TASK_START();
     cilkmerge_par(A, A + quarter - 1, B, B + quarter - 1, tmpA, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
OMPTR_BEFORE_TASK();
...
{
     OMPTR_TASK_START();
     cilkmerge_par(C, C + quarter - 1, D, low + size - 1, tmpC, &omptr_bb_id);
     OMPTR_TASK_END();
}
OMPTR_AFTER_TASK();
...
#pragma omp taskwait
OMPTR_AFTER_TASKWAIT();
     cilkmerge_par(tmpA, tmpC - 1, tmpC, tmpA + size - 1, A, &omptr_bb_id);
...
{
     OMPTR_INIT();
	bots_message("Computing multisort algorithm (n=%d) \n", bots_arg_size);
	#pragma omp parallel
	#pragma omp single nowait
     {
	     ...
	     #pragma omp task untied firstprivate(omptr_bb_id)
          {
               OMPTR_TASK_START();
	          cilksort_par(array, tmp, bots_arg_size, &omptr_bb_id);
               OMPTR_TASK_END();
          }
          OMPTR_AFTER_TASK();
