...
#ifndef FORCE_TIED_TASKS
void nqueens(int n, int j, char *a, int *solutions, int depth, int* omptr_bb_id_ptr)
#else
void nqueens(int n, int j, char *a, int depth)
#endif
{
		...
		if ( depth < bots_cutoff_value ) {
 			...
 			#pragma omp task untied firstprivate(omptr_bb_id)
			{
				OMPTR_TASK_START();
	  			...
	  			if (ok(j + 1, b))
#ifndef FORCE_TIED_TASKS
	       				nqueens(n, j + 1, b,&csols[i],depth+1,&omptr_bb_id);
#else
		       			nqueens(n, j + 1, b,depth+1);
#endif
				OMPTR_TASK_END();
	...
	for (i = 0; i < n; i++) {
 		...
 		#pragma omp task untied firstprivate(omptr_bb_id)
		{
			OMPTR_TASK_START();
	  		...
	  		if (ok(j + 1, b))
#ifndef FORCE_TIED_TASKS
       				nqueens(n, j + 1, b,&csols[i],depth,&omptr_bb_id); //FIXME: depth or depth+1 ???
#else
...
void find_queens (int size)
{
	...
	total_count=0;
        ...
        bots_message("Computing N-Queens algorithm (n=%d)\n", size);
	#pragma omp parallel
	{
		#pragma omp single
		{
...
#ifndef FORCE_TIED_TASKS
			nqueens(size, 0, a, &total_count,0,&omptr_bb_id);
#else
			nqueens(size, 0, a, 0);
#endif
