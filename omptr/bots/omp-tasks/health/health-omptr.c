...
void sim_village_par(struct Village *village, int* omptr_bb_id_ptr)
{
   OMPTR_NEW_CONTEXT();
   struct Village *vlist;
...
#pragma omp task untied firstprivate(omptr_bb_id)
      {
         OMPTR_TASK_START();
         sim_village_par(vlist, &omptr_bb_id);
         OMPTR_TASK_END();
      }
         OMPTR_AFTER_TASK();
         vlist = vlist->next;
...
{
   OMPTR_TASK_START();
   for (i = 0; i < sim_time; i++) sim_village_par(top, &omptr_bb_id); 
   OMPTR_TASK_END();
}  // end task 
   OMPTR_AFTER_TASK();
