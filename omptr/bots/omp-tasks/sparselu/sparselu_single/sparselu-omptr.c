   ...
   for (kk=0; kk<bots_arg_size; kk++) 
   {
      ...
      for (jj=kk+1; jj<bots_arg_size; jj++)
         if (BENCH[kk*bots_arg_size+jj] != NULL) {
            OMPTR_BEFORE_TASK();
            #pragma omp task untied firstprivate(kk, jj, omptr_bb_id) shared(BENCH)
            {
               OMPTR_TASK_START();
      ...
      for (ii=kk+1; ii<bots_arg_size; ii++) 
         if (BENCH[ii*bots_arg_size+kk] != NULL) {
            OMPTR_BEFORE_TASK();
            #pragma omp task untied firstprivate(kk, ii, omptr_bb_id) shared(BENCH)
            {
               OMPTR_TASK_START();
      ...
      for (ii=kk+1; ii<bots_arg_size; ii++)
         if (BENCH[ii*bots_arg_size+kk] != NULL)
            for (jj=kk+1; jj<bots_arg_size; jj++)
               if (BENCH[kk*bots_arg_size+jj] != NULL) {
                  OMPTR_BEFORE_TASK();
                  #pragma omp task untied firstprivate(kk, jj, ii, omptr_bb_id) shared(BENCH)
                  {
                     OMPTR_TASK_START();
